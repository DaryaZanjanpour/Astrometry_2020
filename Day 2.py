# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 14:41:53 2019
...
#FLAT
d1 = Path('Proserpina/26Proserpina/20120120/Day 2/FLAT')
day1 = os.listdir(d1)
...
#Dark
d1 = Path('Proserpina/26Proserpina/20120120/Day 2/DARK')
day1 = os.listdir(d1)
headerd1 = fits.open(d1/ day1[0])
#getting data and median for the Proserpina data
for i in range(0,int(len(day1))):
...
#Bias
d1 = Path('Proserpina/26Proserpina/20120120/Day 2/BIAS')
day1 = os.listdir(d1)
headerd1 = fits.open(d1/ day1[0])
#getting data and median for the Proserpina data
for i in range(0,int(len(day1))):
...
#DATA
d1 = Path('Proserpina/26Proserpina/20120120/Day 2/DATA')
day1 = os.listdir(d1)
headerd1 = fits.open(d1/ day1[0])
#getting data and median for the Proserpina data
for i in range(0,int(len(day1))):
...
def finalcenteroid():
    ...
    finalcenteroy= ([])
    c= 0
    for x in range (60 , 1200): #(24,2024)
                ...
                if (data[y-1][x]<Border):
                    if (data[y][x-1]<Border):
                        ...
                        if (len(newbornfence)>0):
                            ...
                            c= c+0
    for x in range (1300 , 2024): #(24,2024)
        for y in range(24 , 2024): #(24,2024)
            if (data[y][x]>Border):
                if (data[y-1][x]<Border):
                    if (data[y][x-1]<Border):
...
def sigmaclip ( xpixels , ypixels , xresiduals , yresiduals , centx , centy ):
    N = len( xresiduals )
    mfp = 3
    dof = N - mfp
    clip = 2.0 # mu +/− clip∗sigma
    mux, muy, sigmax , sigmay = 0. , 0. , 0. , 0.
    xclippings , yclippings , removed = [ ] , [ ] , [ ]
    mux = np.sum( xresiduals)/N
    muy = np.sum( yresiduals)/N
    for x in range(len(xresiduals)):
        sigmax = sigmax+((xresiduals[x]-(mux))**2)
        sigmax = np.sqrt(sigmax/(dof))
        clipupx = mux+( clip*sigmax)
        clipdnx = mux-( clip*sigmax)
        for z in range ( len ( xresiduals )):
            if ( xresiduals [ z ] > clipupx ) or ( xresiduals [ z ] < clipdnx ):
                xclippings.append(z)
                removed.append(z)
        for x in reversed (np . sort ( xclippings )):
            del xpixels [ x ] , ypixels [ x ] , xresiduals [ x ] , yresiduals [ x ] , centx [ x ] , centy[ x ]
        for y in range ( len ( yresiduals )): 
            sigmay += ( yresiduals [ y]-muy)**2
        sigmay = np . sqrt (sigmay/( dof ))
        clipupy = muy+(clip*sigmay)
        clipdny = muy-(clip*sigmay)
        for z in range ( len ( yresiduals )):
            if ( yresiduals [ z ] > clipupy ) or ( yresiduals [ z ] < clipdny ):
                yclippings.append(z)
                removed.append(z)
        for x in reversed (np.sort( yclippings )):
            del xpixels [ x ] , ypixels [ x ] , xresiduals [ x ] , yresiduals [ x ] , centx [ x ] , centy [ x ]
        return xpixels , ypixels , xresiduals , yresiduals , centx , centy , np.sort(removed)

#print(sigmaclip ( "xlist=", xlist , "ylist=", ylist , "xoffset=", xoffset , "yoffset=" ,yoffset ,"x centeroid=", xstar , "y centroid=", ystar ))

xpixels= sigmaclip (xpixels, ypixels , xresiduals , yresiduals , centx , centy )[0]
ypixels= sigmaclip ( xpixels , ypixels , xresiduals , yresiduals , centx , centy )[1]
xresiduals= sigmaclip ( xpixels , ypixels , xresiduals , yresiduals , centx , centy )[2]
yresiduals= sigmaclip ( xpixels , ypixels , xresiduals , yresiduals , centx , centy )[3]
centx= sigmaclip ( xpixels , ypixels , xresiduals , yresiduals , centx , centy )[4]
centy= sigmaclip ( xpixels , ypixels , xresiduals , yresiduals , centx , centy )[5]


print(len(xpixels))
print(len(ypixels))
print(len(xresiduals))
print(len(yresiduals))
print(len(centx))
print(len(centy))
...
plt.ylim(0, 2000)     
plt.xlim(0, 2000)     
plt.scatter(xpixels, ypixels,c='red', marker='^', label="centroid positions")
plt.scatter(centx, centy,c='blue',marker='v', label="star positions")
plt.legend(loc='best')
plt.show()
...
plt.xlabel("x pixel")
plt.ylabel("y pixel")        
plt.scatter(xpixels, xresiduals,c='red', marker='v', label="x")
plt.scatter(ypixels, yresiduals,c='blue',marker='^', label="y")
plt.legend(loc='best')
plt.show()
           

#print("sigmaclip=",sigmaclip(xlist, ylist,xoffset,yoffset,xstar,ystar))

#xlist, ylist, xoffset, yoffset, xstar, ystar= sigmaclip(xlist, ylist,xoffset,yoffset,xstar,ystar)
    ...
    for k in range (len(xcent)):
        ...
        for l in range(len(ycent)):
            ...
            for n in range(len(ypix)):
                ...
                D[n][1]=ppr*ypix[n]
                D[n][2]=1
            BtBinv = np.linalg.inv(np.dot(np.transpose (B) ,B))
            Bta = np.dot(np.transpose (B) ,a)
            cx = np.dot(BtBinv , Bta)
            DtDinv = np.linalg.inv(np.dot(np.transpose (D) ,D))
